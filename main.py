...
def check_version():
    ...
    if r.text.rstrip() == version:
        ...
        print(r.text)
        return f"{Fore.LIGHTMAGENTA_EX}Đã có phiên bản mới: {r.text}\nCập nhật lại code tại đây: https://github.com/TheAxes/Advance-Auto-Owo{Fore.RESET}"
        ...
        "embeds": [
            {
                ...
                "footer": {
                    "text": "Thông Báo OwO",
                    "icon_url": "https://i.imgur.com/X8C6etz.png",  # Replace with your footer icon URL
                },
            }
        ],
    }
...
"""
    await client.change_presence(
        ...
        activity=discord.Activity(
            ...
            name="Đen Đá",
            details="Đen đá khum đường lói chung là đắng.",
        ...
        if message.author.id == 408785106942164992:
                ...
                if message.channel.id in owochannels or message.channel.id == owodm:
                    ...
                    or ("link" in message.content or "https://owobot.com" in message.content)
                        ...
                        content=f"<@482386409324544020> Captcha Thông Báo!",
                        description=f"Một captcha đã được phát hiện!\n*Tin nhắn captcha*: [Đi đến tin nhắn]({captchamsg})", image_url="https://i.imgur.com/91FHmJh.png")
                        ...
                        if solution.split("|")[1] == "image":
                            ...
                            try:
                                    ...
                                    if "I have verified" in verification_message.content:
                                        ...
                                        sendhook(hook_url=captcha_hook_url, content=f"<@482386409324544020> Captcha Thông Báo!", description=f"<:ngg_On:1337609352080785429> Captcha đã được giải quyết!", image_url="https://i.imgur.com/91FHmJh.png")
                                        await channel.send(f"{prefix}autoowo")
                                    else:
                                        sendhook(hook_url=captcha_hook_url, image_url="https://i.imgur.com/91FHmJh.png" ,content=f"<@482386409324544020> Captcha Thông Báo!", description=f"<:ngg_Off:1337609350050484276> Một captcha không thể được giải quyết, bot đã bị dừng!")
                                        time.sleep(4)
                                        sys.exit()
                            except asyncio.TimeoutError:
                                print(f"{Fore.RED}[Timeout] captcha timed out.{Fore.RESET}")
                                sendhook(hook_url=captcha_hook_url, image_url="https://i.imgur.com/91FHmJh.png", content=f"<@482386409324544020> Captcha Thông Báo!",description=f"<:ngg_Off:1337609350050484276> Một captcha không thể được giải quyết, bot đã bị dừng!, Lý do: Captcha mất quá lâu để giải quyết")
                                time.sleep(5)
                                sys.exit()
...
async def autohunter():
    ...
    await channel.send("hh")
    await asyncio.sleep(15)
    await channel.send("hb")
...
async def autosell():
    ...
    await channel.send(f"hsell {config['settings']['animal_types']}")
...
async def autoslot():
    ...
    await channel.send(f"hs {amount}")

@tasks.loop(minutes=5)
async def autocf():
    channel = client.get_channel(owochannel)
    amount = random.choice(config["settings"]["autocoinflip_amount"])
    await channel.send(f"hcf {amount}")
...
async def autohuntbot():
    ...
    if not get_entry()[3] - time.time() <= 0:
        ...
        if amount_message.attachments[0].url:
            ...
            seconds = minutes * 60
            sendhook(hook_url=huntbot_hook_url, content="HuntBot Alert!!", description=f"Huntbot bắt đầu !!\n[Đi đến tin nhắn]({huntbot_msg.jump_url})", image_url="https://images-ext-1.discordapp.net/external/r-T0CN-zkuhykmnsWyy6gRSkZyAb-mm7EDeH-lUi_w8/https/cdn.discordapp.com/emojis/459996048379609098.png?format=webp&quality=lossless&width=160&height=160")
...
async def help(ctx):
    ...
    [Fix By @sonic2109]
      [Help Command]
⁙ {prefix}autoowo - Bắt đầu Farming OwO tự động
⁙ {prefix}stopautoowo - Dừng Farming OwO
⁙ {prefix}chhservicekey (new key) - Thay đổi khóa dịch vụ hcaptcha
⁙ {prefix}chtexttoimagekey (new key) - hay đổi khóa dịch vụ texttoimage
⁙ {prefix}balance - Trả về số dư của từng dịch vụ
⁙ {prefix}info - Trả về thông tin instance cùng các chi tiết khác
```
'''
...
async def chhservicekey(ctx, key=None):
    if key is None:
        ...
        await ctx.send("`Đã cập nhật khóa hcaptcha_key thành công`")
...
async def chtexttoimagekey(ctx, key=None):
    if key is None:
        ...
        await ctx.message.delete()
        await ctx.send("`Đã cập nhật TextToImage_Key Key thành công`")

@client.command()
async def balance(ctx):
    message = await ctx.send('`<a:Loading:1343272327520981104> Đang tải...`')
    hbal = fetch_hcaptcha_balance()
    txtbal = fetch_texttoimage_balance()
    msg = f'''
```ini
...
@client.command()
async def info(ctx):
    message = await ctx.reply("`<a:Loading:1343272327520981104> Đang tải...`")
